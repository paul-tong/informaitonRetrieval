...
public class IndexGenerater {
     ...
     */
    IndexGenerater(String indexDir) throws IOException {
     ...
     */
    public void query(String queryTerm, int topK, String resultDir) throws IOException, ParseException {
		...
		System.out.println("Top " + topK + " results for querying [" + queryTerm + "] :");
    ...
    public static void main(String[] args) throws IOException, ParseException {
    	...
    	IndexGenerater indexer = null;
		try {
		    indexer = new IndexGenerater(indexDir);
		} catch (Exception ex) {
		...
		List<String> queryList = Arrays.asList("Robotic space missions", "Mars exploration", "unmanned spacecraft", "Planetary moons", "Satellites in Space");
		/*for (String queryTerm : queryList) {
	    	String queryTermDir = queryTerm.toLowerCase().replace(' ', '-');
	    	String resultDir = RESULT_DIR + "/" + queryTermDir + ".txt";
			indexer.query(queryTerm, 100, resultDir); // query top 100 results for given term	
		}*/
		
		// get query expansion
		/*for (String queryTerm : queryList) {
	    	String queryTermDir = queryTerm.toLowerCase().replace(' ', '-');
	    	String expansionDir = EXPANSION_DIR + "/" + queryTermDir + ".txt";
		...
		for (String queryTerm : queryList) {
	    	String queryTermDir = queryTerm.toLowerCase().replace(' ', '-');
	    	String resultDir = RESULT_EXPANSION_DIR + "/" + queryTermDir + ".txt";
	    	String expansionPath = EXPANSION_DIR + "/" + queryTermDir + ".txt";
